		...
		<div id="cart-container" class="cart-container <?php if (WC()->cart->cart_contents_count < 1) { ?>empty<?php } ?>">
			...
			<?php if (WC()->cart->cart_contents_count>0) : ?>
				...
				<div class="subtotal item">
				    <?php _e('Subtotal', THB_THEME_NAME); ?><?php echo WC()->cart->get_cart_total(); ?>
				</div>
			
				<div class="buttons item">
					<a href="<?php echo esc_url( WC()->cart->get_cart_url() ); ?>"><i class="fa fa-chevron-left"></i> <?php _e('Continue', THB_THEME_NAME); ?></a>
			
					<a href="<?php echo esc_url( WC()->cart->get_checkout_url() ); ?>"><?php _e('Checkout', THB_THEME_NAME); ?> <i class="fa fa-chevron-right"></i></a>
				</div>
			</div>
			<?php else: ?>
				<div class="table">
...
function thb_quick_cart() {
 ...
 ?>
<a id="quick_cart" href="<?php echo esc_url( WC()->cart->get_cart_url() ); ?>" title="<?php _e('View your shopping cart',THB_THEME_NAME); ?>">
...
function thb_woocomerce_ajax_cart_update($fragments) {
	if(class_exists('woocommerce')) {
		...
		?>
			...
			jQuery(function($){
				favicon.badge(<?php echo WC()->cart->cart_contents_count; ?>);
			});// ]]>
			</script>
		<?php
