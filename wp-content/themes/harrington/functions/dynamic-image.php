 ...
 * @return mixed|string
 */
function harrington_document_root($path) {
...
if ((function_exists('is_multisite') && is_multisite()) || ($single_site = true)) {
    ...
    function harrington_image_resize($img_url, $width, $height, $crop = true, $quality = 100) {
        ...
        if (isset($file_path['host']) && $_SERVER['HTTP_HOST'] != $file_path['host'] && $file_path['host'] != '') {   // The image is not locally hosted
            ...
            if (!file_exists($copy_to_file)) {
                $unique_filename = wp_unique_filename($upload_dir['path'],
                        $file_base);
                // Using the HTTP API instead of our own CURL calls...
                $remote_content = wp_remote_request($img_url,
                        array(
                ...
                if (is_wp_error($remote_content)) {
                    ...
                    wp_upload_bits($unique_filename,
                            null,
                            $remote_content['body']);
            ...
            $string = $expath;
            $find = '/files/';
            $findit = strpos($string,
                    $find);
            //echo "</br> Findit=>".$findit;
            if ($findit === false) {
                $file_path = harrington_document_root($file_path['path']) . $file_path['path'];
            } else {
                $expath = $file_path['path'];
                $nefilepath = explode("/files",
                        $expath);
        ...
        if ($source[1] > $width || $source[2] > $height) {
            ...
            if (file_exists($cropped_img_path)) {
                $cropped_img_url = str_replace($upload_dir['basedir'],
                        $upload_dir['baseurl'],
                        $cropped_img_path);
                $resized_image = array(
                    'url' => $cropped_img_url,
            ...
            if ($crop == false) {
                ...
                $proportional_size = wp_constrain_dimensions($source[1],
                        $source[2],
                        $width,
                        $height);
                ...
                if (file_exists($resized_img_path)) {
                    $resized_img_url = str_replace($upload_dir['basedir'],
                            $upload_dir['baseurl'],
                            $resized_img_path);
                    $resized_image = array(
                        'url' => $resized_img_url,
...
function harrington_thumbnail_resize($attach_id = null, $img_url = null, $width, $height, $crop = false, $jpeg_quality = 90) {
    ...
    if ($image_src[1] > $width || $image_src[2] > $height) {
        ...
        if (file_exists($cropped_img_path)) {
            $cropped_img_url = str_replace(basename($image_src[0]),
                    basename($cropped_img_path),
                    $image_src[0]);
            $vt_image = array(
                'url' => $cropped_img_url,
                'width' => $width,
        ...
        if ($crop == false) {
            ...
            // checking if the file already exists
            if (file_exists($resized_img_path)) {
                $resized_img_url = str_replace(basename($image_src[0]),
        ...
        if (!is_wp_error($img)) {
            ...
            $path = str_replace(basename($image_src[0]),
                    '',
                    $image_src[0]);
            $filename = $img->generate_filename('final' . $width,
                    $path . '/',
                    NULL);
            $image_detail = $img->save($filename);
        }
        $new_img = str_replace(basename($image_src[0]),
                basename($image_detail['path']),
                $image_src[0]);
        $vt_image = array(
            'url' => $new_img,
